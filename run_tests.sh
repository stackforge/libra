...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./libra/tests"
    fi
  ...
  if [ setup.cfg -nt libra.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='libra/*' --omit='libra/openstack/common/*' -d covhtml -i
  fi
