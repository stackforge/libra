...
swift_container = "libra_git_repo"
...
# We need a list of files to ignore (with tar -X). First we put .git/
# on the list since we never want that diretory. Next we use the
# documented untracked_files attribute to get a list of untracked
# files and directories. Then we use a regex hack with "git status"
...
if not done:
    print "ERROR: swift does not acknowledge upload after 120 seconds:\n%s" % output
    exit(1)
