...
class LoadBalancersController(RestController):
    ...
    def post(self, body=None):
        ...
        if body.protocol and body.protocol.lower() == 'galara':
            is_galera = True
        else:
            is_galera = False
        ...
        if is_galera and num_galera_primary_nodes > 1:
            raise ClientSideError(
                'Galera load balancer may have only 1 primary node'
            )
        elif is_galera and num_galera_primary_nodes == 0:
            raise ClientSideError(
                'Galera load balancer requires 1 primary node'
            )
