...
class UsageStats(object):
    ...
    def gather_stats(self):
        ...
        try:
            ...
            self.logger.exception('Uncaught exception during stats collection')

        # Need to restart timer after every stats cycle
        self.logger.info('{total} lb device stats queried, {fail} failed'
    ...
    def _exec_stats(self):
        failed = 0
        node_list = []
        self.logger.info('Running stats')
        ...
        with db_session() as session:
            ...
            total = len(devices)
            if total == 0:
                self.logger.info('No LBs to gather stats from')
                return (0, 0)
            for lb in devices:
    ...
    def _update_stats(self, results):
        with db_session() as session:
            ...
            if lbs is None:
                session.rollback()
                self.logger.error('No Loadbalancers found when updating stats')
                return
    ...
    def start_stats_sched(self):
        ...
        self.logger.info('LB stats timer sleeping for {secs} seconds'
                         .format(secs=sleeptime))
        self.stats_timer = threading.Timer(sleeptime, self.update_stats, ())
