...
class BillingStats(object):
    ...
    def _exec_exists(self):
        with db_session() as session:
            ...
            if updated is not None:
                # Not time yet
                LOG.info('Not time to send exists yet {0}'.format(exp_time))
                session.rollback()
                return
    ...
    def _exec_usage(self):
        with db_session() as session:
            ...
            LOG.info('Exp = {0}'.format(exp))

            if isinstance(start, tuple):
                start = start[0]
                LOG.info('Start Tuple = {0}'.format(start))

            if start is not None and start > exp:
                # Not time yet
                LOG.info('Not time to send usage yet {0}'.format(exp))
                session.rollback()
                return
