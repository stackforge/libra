        ...
        with db_session() as session:
            ...
            if not device_id:
                ...
                if vip is not None:
                    ...
                    devices.join(Device.vip).filter(Vip.ip == vip_num)
                else:
                    # If not filtering by Vip, still show Vip if one exists
                    devices.outerjoin(Device.vip)
                ...
                if status is not None:
                    ...
                    devices.filter(Device.status == status)
                if name is not None:
                    # Search devices by name, should only return one
                    devices.filter(Device.name == name)
                if ip is not None:
                    # Search devices by IP, should only return one
                    devices.filter(Device.floatingIpAddr == ip)
