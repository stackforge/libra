...
def _send_usage(event_type, start, stop):
    ...
    with db_session() as session:
        ...
        # Track the oldest value in case we need it below. Note that
        # the datetime values come out of sqlalchemy as tuples, so we
        # have to just grab the first value.
        oldest = session.query(Stats.period_end).\
            order_by(Stats.id.asc()).first()
        if isinstance(oldest, tuple):
            oldest = oldest[0]
            LOG.info("Oldest from Stats Table = {0}".format(oldest))

        if oldest is None:
            # No Stats at all
            LOG.info("Billing usage statistics table is Empty")
            session.rollback()
            return
        ...
        if start is None:
            ...
            LOG.info("OLDEST = {0}".format(start))
        ...
        total = session.query(Stats).\
            ...
            count()
        if total == 0:
            LOG.info("No Billing usage statistics to send between {0} and {1}"
                     .format(start, stop))
            session.rollback()
            return

        LOG.info("Found {0} total usage statistics to send".format(total))
        ...
        LOG.info("Start BL = {0}".format(start))
        LOG.info("End BL = {0}".format(end))
        LOG.info("Stop BL = {0}".format(stop))
        count = 0
        while end <= stop:
            # Loop through all N periods up to the current period
            # sending usage notifications to MnB
            LOG.info("Start = {0}".format(start))
            LOG.info("End = {0}".format(end))
            stats = session.query(
                Stats.lbid,
            ...
            if stats is None:
                LOG.info("No Billing usage statistics to send "
                         "for period {0} to {1}".format(prev_start, prev_end))
                continue
            ...
            for lb in stats:
                lbid, byte_count = lb
                LOG.info("LBID = {0}, BYTES = {1}".format(lbid, byte_count))

                if lbid not in loadbalancers:
                    LOG.error("Loadbalancer {0} not found in DB "
                              "not sending usage data".format(lbid))
                    continue
                ...
                payload = {
                    ...
                    "audit_period_ending": audit_period_ending,
                    "record_type": "event",
                    "display_name": loadbalancers[lbid]["name"],
                    "id": lbid,
                    "type": "lbaas.std",
                    "type_id": 1,
                    "tenant_id": loadbalancers[lbid]["tenant_id"],
                    "state": loadbalancers[lbid]["status"],
                    "state_description": loadbalancers[lbid]["status"],
                    "metrics": {
                        "metric_name": "lbaas.network.outgoing.bytes",
                        "metric_type": "gauge",
                        "metric_units": "BYTES",
                        "metric_value": byte_count
                    }
                }
