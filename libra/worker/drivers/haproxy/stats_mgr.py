...
class StatisticsManager(object):
    ...
    """
    - IF HAProxy restart
      - SAVE UNREPORTED stats

    - IF STATS request received
      - IF UNREPORTED value exists
        - SET data.bytes_out = current bytes out + UNREPORTED bytes out
        - SET data.start = UNREPORTED start
        - SET data.end = current timestamp
        - RETURN data
        - CLEAR UNREPORTED values
        - SAVE last reported values
      - ELSE
        - SET data.bytes_out = current bytes out
        - SET data.start = last reported timestamp + 1 sec
        - SET data.end = current timestamp
        - RETURN data
        - SAVE last reported values
    """
