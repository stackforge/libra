...
class TestStatisticsManager(TestCase):
    ...
    def testReadNoStatsFile(self):
        ...
        self.assertEquals(self.mgr.get_tcp_last_bytes(), 0)
        self.assertEquals(self.mgr.get_http_last_bytes(), 0)
        self.assertEquals(self.mgr.get_unreported_start(), None)
        self.assertEquals(self.mgr.get_unreported_end(), None)
        self.assertEquals(self.mgr.get_tcp_unreported_bytes(), 0)
        self.assertEquals(self.mgr.get_http_unreported_bytes(), 0)
    ...
    def testSaveLastReported(self):
        ...
        tcp_bytes = 1024
        http_bytes = 2048
        self.mgr.save_last_reported(start_ts, end_ts, tcp_bytes, http_bytes)
        self.mgr.read()
        self.assertEquals(self.mgr.get_last_start(), start_ts)
        self.assertEquals(self.mgr.get_last_end(), end_ts)
        self.assertEquals(self.mgr.get_tcp_last_bytes(), tcp_bytes)
        self.assertEquals(self.mgr.get_http_last_bytes(), http_bytes)
    ...
    def testSaveUnreported(self):
        ...
        self.assertEquals(self.mgr.get_tcp_unreported_bytes(), tcp_bytes)
        self.assertEquals(self.mgr.get_http_unreported_bytes(), http_bytes)
