...
if _using_eventlet_green_select:
    logging.debug("eventlet")
    SelectPoll = _SelectSelect
else:
    SelectPoll = select.poll  # use the default
...
class Server(BaseClientServer):
    ...
    def getQueue(self):
        ...
        """
        ret = []
        for q in [self.high_queue, self.normal_queue, self.low_queue]:
            ret += q
        return ret
            ...
            for q in [self.high_queue, self.normal_queue, self.low_queue]:
                for job in q:
                    if handle == job.handle:
                        return self._cancelJob(request, job, q)
